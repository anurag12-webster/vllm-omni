...
"""
E2E Online tests for MOSS-TTS model with text input and audio output.
...
def get_stage_config():
    """Get the stage config path for MOSS-TTS."""
    return str(
...
def omni_server():
    """Start vLLM-Omni server with MOSS-TTS model."""
...
# Minimum expected audio size for a short sentence (~1 second of 24kHz mono WAV)
MIN_AUDIO_BYTES = 10000


class TestMossTTS:
    """E2E tests for MOSS-TTS model."""
    ...
    def test_speech_english_basic(self, omni_server) -> None:
        ...
        assert len(response.content) > MIN_AUDIO_BYTES, (
            ...
            f"expected at least {MIN_AUDIO_BYTES}"
        )
